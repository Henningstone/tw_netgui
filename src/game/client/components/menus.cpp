...
void CMenus::OnReset()
{
	// TODO: NetGui. This is not pretty. But useful :P
	Client()->m_NetGuiUIRect.clear();
	Client()->m_NetGuiLabel.clear();
	Client()->m_NetGuiButtonMenu.clear();
}

void CMenus::OnMessage(int MsgId, void *pRawMsg)
{
	if(MsgId == NETMSGTYPE_SV_NETGUI_REMOVEELEMENT) // TODO: NetGui
	{
		...
		case NETMSGTYPE_SV_NETGUI_UIRECT:
			...
			{
				if(Client()->m_NetGuiUIRect[i].m_ID == pMsg->m_ID)
				{
					dbg_msg("NETGUI", "removing UIRect with ID:%d", pMsg->m_ID);
					Client()->m_NetGuiUIRect.remove_index(i);
				}
			}
			break;
		case NETMSGTYPE_SV_NETGUI_LABEL:
			for(int i = 0; i < Client()->m_NetGuiLabel.size(); i++)
			{
				...
				if(Client()->m_NetGuiLabel[i].m_ID == pMsg->m_ID)
				{
					dbg_msg("NETGUI", "removing Label with ID:%d", pMsg->m_ID);
					Client()->m_NetGuiLabel.remove_index(i);
				}
			}
			break;
		case NETMSGTYPE_SV_NETGUI_BUTTONMENU:
			...
			{
				if(Client()->m_NetGuiButtonMenu[i].m_ID == pMsg->m_ID)
				{
					dbg_msg("NETGUI", "removing ButtonMenu with ID:%d", pMsg->m_ID);
					Client()->m_NetGuiButtonMenu.remove_index(i);
				}
			}
			break;
		}
	...
	else if(MsgId == NETMSGTYPE_SV_NETGUI_UIRECT)
	{
		...
		for(int i = 0; Client()->m_NetGuiUIRect.size(); i++)
		{
			if(Client()->m_NetGuiUIRect[i].m_ID == e.m_ID)
				Client()->m_NetGuiUIRect.remove_index(i);
		}

		// save the element and resort the list
		Client()->m_NetGuiUIRect.add(e);
		Client()->SortNetGuiList<CNetMsg_Sv_NetGui_UIRect>(Client()->m_NetGuiUIRect);

		dbg_msg("NETGUI", "RECEIVED RECT ID:%d, num=%i", e.m_ID, Client()->m_NetGuiUIRect.size());
		for(int i = 0; i < Client()->m_NetGuiLabel.size(); i++)
			dbg_msg("NETGUI", " Rect IDs: %d", Client()->m_NetGuiUIRect[i].m_ID);
	}
	else if(MsgId == NETMSGTYPE_SV_NETGUI_LABEL)
	{
		...
		// check for duplicated IDs and overwrite them
		for(int i = 0; Client()->m_NetGuiLabel.size(); i++)
		{
			if(Client()->m_NetGuiLabel[i].m_ID == e.m_ID)
				Client()->m_NetGuiLabel.remove_index(i);
		}

		// save the element and resort the list
		Client()->m_NetGuiLabel.add(e);
		Client()->SortNetGuiList<CNetMsg_Sv_NetGui_Label>(Client()->m_NetGuiLabel);

		dbg_msg("NETGUI", "RECEIVED LABEL ID:%d, num=%i", e.m_ID, Client()->m_NetGuiLabel.size());
		for(int i = 0; i < Client()->m_NetGuiLabel.size(); i++)
			dbg_msg("NETGUI", " Label IDs: %d", Client()->m_NetGuiLabel[i].m_ID);
	}
	else if(MsgId == NETMSGTYPE_SV_NETGUI_BUTTONMENU)
	{
		...
		// check for duplicated IDs and overwrite them
		for(int i = 0; Client()->m_NetGuiButtonMenu.size(); i++)
		{
			if(Client()->m_NetGuiButtonMenu[i].m_ID == e.m_ID)
			{
				Client()->m_NetGuiButtonMenu.remove_index(i);
			}
		}
		...
		// save the element and resort the list
		Client()->m_NetGuiButtonMenu.add(e);
		Client()->SortNetGuiList<CNetMsg_Sv_NetGui_ButtonMenu>(Client()->m_NetGuiButtonMenu);

		dbg_msg("NETGUI", "RECEIVED BUTTON ID:%d, num=%i", e.m_ID, Client()->m_NetGuiButtonMenu.size());
		for(int i = 0; i < Client()->m_NetGuiButtonMenu.size(); i++)
			dbg_msg("NETGUI", " Button IDs: %d", Client()->m_NetGuiButtonMenu[i].m_ID);
	}
}
